...
def sum_to(n: int) -> int:
    """
    Demonstrate lazy eval for numbers() generator
    numbers() will only print to the point i==n
    (strict eval would have to print all numbers generated by numbers())
    """
    sum: int = 0
    for i in numbers():
        if i == n:
...
__test__ = {
    ...
    # Higher Order functions :
         # Takes a fn as an arg or returns a fn
    """Higher Order Functions. 
...
Wrap-prcess-unwrap
Create key-value pair -> (29.87, (2000, 29.87)) -- wrap
Default max behavior gets tuple with largest key -- process
Index to get original typle -- unwrap
...
Some functional languages have something like this snd fn (and fst)
Defining like this is a little more readable but same as subscript
>>> snd= lambda x: x[1]
...
(2007, 33.5)
    ...
    """,
    'sum_non_strict':
    # Non-strict (lazy) evalution
        # Eval can be deferred until needed
        # In python, and/or/if-else are non strict
        # Generators functions and expressions are also non strict
